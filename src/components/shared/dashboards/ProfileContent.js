...
const ProfileContent = () => {
		...
		const fetchUserData = async () => {
			try {
				...
				if (res.ok) {
					...
					const newSkillsOptions = existingExpertise.reduce(
						(acc, skill) => {
	...
	const handleSkillsChange = (selectedOptions) => {
		const expertise = selectedOptions
	...
	const selectedSkills = skillsOptions.filter((option) =>
				...
				<div>
					...
					<CreatableSelect
						...
						onCreateOption={(inputValue) => {
							...
							setFormData({
								...formData,
								expertise: [...formData.expertise, inputValue],
							}); // Changed from "skills" to "expertise"
						}}
					/>
				</div>
				<div>
