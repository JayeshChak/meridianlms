...
const UserDetails: React.FC<UserDetailsProps> = ({ user_id }) => {
		...
		const fetchUserDetails = async () => {
			...
			try {
				...
				if (!response.ok) {
					...
					throw new Error(
						errorData.error || "Failed to fetch User details."
					);
				}
	...
	const renderSocialLink = (
		...
		const icons: {
			[key in keyof UserDetailsType["socials"]]: JSX.Element;
		} = {
			facebook: (
				<svg
			...
			{UserDetails.enrolled_courses &&
				UserDetails.enrolled_courses.length > 0 && (
					<div className="mt-8">
						...
						<p className="text-gray-700">
							Details about enrolled Courses or experience.
						</p>
					</div>
				)}
...
//       try {
//         const response = await fetch(`/api/Courses/${course_id}`); // Corrected path
//         if (!response.ok) {
//           console.warn(`Course with ID ${course_id} not found.`);
//           return; // Skip if course not found
//         }
//         const courseData = await response.json();
//         setCourses((prev) => ({ ...prev, [course_id]: courseData }));
//       } catch (err: any) {
...
//     UserDetails.enrolled_courses &&
//       UserDetails.enrolled_courses.forEach((course) => {
//         if (!Courses[course.course_id]) {
...
//               {UserDetails.enrolled_courses &&
//               UserDetails.enrolled_courses.length > 0 ? (
//                 UserDetails.enrolled_courses.map((course) => (
...
//                     </div>
//                     <p className="mt-2">
//                       {Courses[course.course_id]?.description ||
...
//                   {UserDetails.wishlist.map((course_id) => (
//                     <li key={course_id}>
//                       {Courses[course_id]?.title || `Course ID: ${course_id}`}
//                     </li>
//                   ))}
//                 </ul>
//               ) : (
