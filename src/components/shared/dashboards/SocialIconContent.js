	...
	const fetchSocialLinks = async () => {
		...
		try {
			const response = await fetch(
				`/api/User/${session.User.id}/socials`,
				{
					method: "GET",
				}
			);
			...
			if (response.ok) {
				...
				showAlert(
					"error",
					errorData.error || "Failed to load social links.",
					"Error"
				);
			}
		} catch (error) {
			...
			showAlert(
				"error",
				"An error occurred while fetching social links.",
				"Error"
			);
		} finally {
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (!response.ok) {
				showAlert(
					"error",
					result.error || "Failed to update social links.",
					"Error"
				);
			} else {
				showAlert(
			...
			showAlert(
				"error",
				"An error occurred while updating social links.",
				"Error"
			);
		} finally {
			setLoading(false); // Stop loading
