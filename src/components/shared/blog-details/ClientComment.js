...
const ClientComment = ({ commits, course_id }) => {
	...
	return (
		<div className="pt-50px pb-15px border-y border-borderColor2 dark:border-borderColor2-dark">
			<h4
				...
				data-aos="fade-up"
			>
				({allCommits?.length}) Comment
				{allCommits?.length > 1 ? "s" : ""}
			</h4>
			<ul>
				{allCommits?.map((commit, index) => (
					<li
						key={commit.id || index}
						className="flex gap-30px mb-10"
					>
						{/* Main comment */}
						...
						<div className="flex-grow">
							...
							{replyCommitIndex === index ? (
								<div className="mt-5">
									...
									<button
										onClick={() =>
											handleReplySubmit(index, commit.id)
										}
