		...
		if (file) {
			...
			img.onload = () => {
				...
				if (
					...
					setImageError(
						"Image dimensions must be 595 x 842 pixels (A4 size) in either orientation."
					);
	...
	const submitCertificateForm = async (base64Image: string) => {
		try {
			...
			const payload = {
				...
				issued_to: session?.User?.id || "",
				title: certificateName,
				expiration_date: "2025-12-31T23:59:59.000Z",
				is_revocable: true,
				unique_identifier: uniqueId, // Use the formatted uniqueId
			...
			router.push(
				`/dashboards/certificates/edit/${responseData.certificate_id}`
			);
