...
interface Certificate {
	id: string;
	certificate_data_url: string; // URL or HTML content
	description: string;
...
const SavedCertificates: React.FC = () => {
	...
	const handlePreview = (certificate: Certificate) => {
		if (certificate.certificate_data_url.startsWith("data:image/")) {
			// Image-based certificate
			Swal.fire({
				title: "Certificate Preview",
				imageUrl: certificate.certificate_data_url,
			...
			Swal.fire({
				title: "Certificate Preview",
				html: certificate.certificate_data_url,
				showCloseButton: true,
				showConfirmButton: false,
				width: "80%",
				heightAuto: true,
