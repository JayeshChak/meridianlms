...
interface ChapterDetailsComponentProps {
	...
	course_id: string; // Adding course_id to identify the course
	onSave: (details: {
		title: string;
		description: string;
		order: string;
	...
	const handleSave = async () => {
		...
		try {
			...
			if (response.ok) {
				...
				setChapterId(result); // Update the chapter_id in the parent component
				...
				showAlert(
					"error",
					`Failed to create chapter: ${errorData.message}`
				);
			}
		} catch (error) {
				...
				{isEditing ? (
					...
					<>
						<div>
							...
							>
								<Tooltip
									content="Click here to create Chapter"
									position="top"
								>
									{localTitle}
								</Tooltip>
