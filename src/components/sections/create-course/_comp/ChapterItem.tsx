...
const ChapterItem: React.FC<ChapterItemProps> = ({
	...
	const fetchQuestionnaires = async () => {
		try {
			...
			const data = await respzonse.json();
	...
	const handleSaveLecture = async (newLecture: Lecture) => {
		try {
			const response = await fetch(
				`/api/Courses/Chapters/Lectures?chapter_id=${chapter_id}`
			);
			if (!response.ok) {
	...
	const removeChapterHandler = async () => {
		...
		try {
			const response = await fetch(
				`/api/Courses/Chapters?id=${chapter_id}`,
				{
					method: "DELETE",
					headers: {
						"Content-Type": "application/json",
		...
		<li className="mb-5 relative">
			<div className="bg-white dark:bg-gray-800 shadow-accordion dark:shadow-accordion-dark rounded-md p-4">
				<ChapterAdd
					...
					initialOrder={
						chapter.order ? parseInt(chapter.order, 10) : 0
					}
			...
			<div className="mt-4 border-t pt-4">
				...
				{chapter.questionnaire_id && (
					<div className="mt-2 p-2 bg-gray-50 rounded">
						<p className="text-sm">
							Assigned Quiz:{" "}
							{availableQuestionnaires.find(
								(q) => q.id === chapter.questionnaire_id
							)?.title || "Loading..."}
						</p>
					</div>
				)}
