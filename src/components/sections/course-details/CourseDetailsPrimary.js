...
import ManageQuestionnaire from "../Questionnaires/ManageQuestionnaire";
...
const CourseDetailsPrimary = ({ id: currentId, type, courseDetails }) => {
    ...
    `;
		...
		visiblePlaceholders.forEach((ph) => {
			const percentX = ((ph.x + 50) / originalWidth) * 100; // X in percentage
			const percentY = ((ph.y + 50) / originalHeight) * 100; // Y in percentage
			const scaledFontSize = Math.max(
				(ph.font_size ?? 16) * scaleFactor,
				12
			); // Prevent too small text
        ...
        <div class="certificate-container">
          ...
          ${Placeholders.map(
            ...
            <div class="placeholder" 
              style="
                top: ${(ph.y / originalHeight) * 100}%;
                left: ${(ph.x / originalWidth) * 100}%;
	...
	const handleCertificateSelect = async () => {
		...
		try {
			...
			// 2️⃣ Fetch the course details to get the certificate_id
			...
			// 3️⃣ Check if the course has a certificate_id
			const storedCertificateId = fetchedCourse?.certificate_id;
			...
			const certificate_data_url = await certificateResponse.json();
			...
			const certificateNumber = `${unique_identifier}-${certificate_id}`;

			// 7️⃣ Replace Placeholders with Actual Data
			const filledPlaceholders = Placeholders.filter(
				(ph) => ph.is_visible
			) // ✅ Only process visible Placeholders
				.map((ph) => {
					switch (ph.key) {
						case "studentName":
						...
						case "sessionStartDate":
							return { ...ph, value: sessionStartDate };
						case "sessionEndDate":
							return { ...ph, value: sessionEndDate };
						case "dateGenerated":
							return {
								...ph,
						...
						case "certificateNumber":
							return { ...ph, value: certificateNumber };
						default:
									...
									<ul className="space-y-[15px] max-w-127">
										...
										>
											...
											<p className="text-sm lg:text-xs 2xl:text-sm font-medium leading-25px lg:leading-21px 2xl:leading-25px text-contentColor dark:text-contentColor-dark">
												...
												andtypesetting industry most of
												the standard.
											</p>
										</li>
										<li
