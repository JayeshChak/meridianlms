...
const AddQuestionnaire: React.FC<AddQuestionnaireProps> = ({ onClose }) => {
	...
	const [questionnaire_id, setQuestionnaireId] = useState<string>("");
	...
	const handleRemoveOption = (indexToRemove: number) => {
		if (options.length > 2) {
			...
			if (correct_answer === options[indexToRemove]) {
				setCorrectAnswer("");
			}
		} else {
	...
	const handleSaveQuestionnaire = async () => {
		...
		try {
			...
			if (!saveResponse.ok) {
				...
				throw new Error(
					errorData.error || "Failed to save questionnaire"
				);
			}
			...
			if (savedData.success) {
				setQuestionnaireId(savedData.data.questionnaire_id);
				showAlert("success", "Questionnaire saved successfully");
				onClose?.();
				router.refresh();
			} else {
				throw new Error(
					savedData.error || "Failed to save questionnaire"
				);
			}
		} catch (error) {
