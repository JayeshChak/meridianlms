...
const ManageQuestionnaire: FC<ManageQuestionnaireProps> = ({
	...
	const [expandedQuestionnaire, setExpandedQuestionnaire] = useState<
		string | null
	>(null);
	...
	}, [questionnaire_id, mode, fetchSingleQuestionnaire, fetchQuestionnaires]);
	...
	const fetchSingleQuestionnaire = async () => {
		try {
			const response = await fetch(
				`/api/Courses/Chapters/Lectures/Questionnaires/${questionnaire_id}`
			);
	...
	const handleDelete = async (id: string) => {
		if (
			...
			try {
				const response = await fetch(
					`/api/Courses/Chapters/Lectures/Questionnaires/?id=${id}`,
					{
						method: "DELETE",
					}
				);
				...
				if (!response.ok) {
					throw new Error(
						result.error || "Failed to delete questionnaire"
					);
				}
				...
				showAlert(
					"error",
					`Failed to delete questionnaire: ${error.message}`
				);
			}
		}
	};
