...
const OrderHistory = () => {
		...
		const fetchOrders = async () => {
			...
			try {
				const response = await fetch(
					`/api/Orders/complete?user_id=${session?.User?.id}&page=${page}&limit=${limit}`
				);
				if (!response.ok) {
	...
	const handleDownloadExcel = async () => {
		...
		try {
			...
			const response = await fetch(
				`/api/Orders/complete?user_id=${session.User.id}&page=1&limit=1000`
			);
			if (!response.ok) {
				throw new Error("Failed to fetch Orders");
			}
			...
			const excelData = allOrders.map((order) => ({
				...
				"User ID": order.unique_identifier || "N/A",
				"Course Name":
					order.items?.map((item) => item.name).join(", ") || "N/A",
				Date: order.created_at
					? new Date(order.created_at).toLocaleDateString()
					: "N/A",
				Price: order.total_amount || "N/A",
				Status:
									...
									Orders.map((order, index) => (
										...
										>
											<th className="px-5px py-10px md:px-5">
												...
												{/* Display first 4 characters of Order ID */}
											</th>
											...
											<td className="px-5px py-10px md:px-5">
												<span
													className={`h-22px inline-block px-3 py-1 rounded-md text-white text-xs font-bold ${
														order.status ===
														"completed"
															? "bg-green-500"
															: order.status ===
