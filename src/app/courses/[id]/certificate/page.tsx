		...
		const fetchCourseDetails = async () => {
			try {
				...
				if (result.data) {
					...
					throw new Error(
						result.message || "Failed to fetch course details"
					);
				}
			} catch (error) {
			...
			<div className="max-w-4xl mx-auto">
				...
				<div className="bg-white rounded-lg shadow-md p-6 mb-6">
					<h2 className="text-xl font-semibold text-gray-800 mb-4 flex items-center">
						<span className="bg-indigo-600 text-black rounded-full w-6 h-6 inline-flex items-center justify-center mr-2">
							1
						</span>
				...
				{isPreviewVisible && (
					<div className="bg-white rounded-lg shadow-md p-6">
						...
						<div className="border-2 border-gray-200 rounded-lg p-4">
							<UserCertificate
								certificate_id={selectedTemplateId}
								userData={userData}
							/>
						</div>
					</div>
