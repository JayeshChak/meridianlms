...
export async function POST(req: Request) {
	try {
		const { items, user_id } = await req.json(); // Assuming items and user_id are passed in the body

		// Ensure price is passed as integer in cents and course_id is included
		...
		const session = await stripe.checkout.sessions.create({
			...
			metadata: {
				user_id, // Pass user_id for order processing
