...
const WishlistContextProvider = ({ children }) => {
	...
	}, [fetchWishlistFromDB, session]);

	// Fetch wishlist from database
	// eslint-disable-next-line react-hooks/exhaustive-deps
	const fetchWishlistFromDB = async (user_id) => {
		try {
...
//     try {
//       const response = await fetch(`http://localhost:3000/api/User/${user_id}?onlyWishlist=false`);
//       const data = await response.json();
//       if (response.ok) {
//         setWishlistProducts(data.wishlist || []); // Set wishlist from database
//       } else {
...
//   if (session) {
//     // Call function to fetch wishlist for a logged-in User (pass the user_id)
//     const user_id = session?.User?.id; // Replace with the logged-in User's ID
...
//   const deleteProductFromWishlist = async (productId, user_id) => {
//     try {
